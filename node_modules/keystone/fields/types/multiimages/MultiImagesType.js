...
/**
 * MultiImages FieldType Constructor
 */
function multiimages (list, path, options) {
	this._underscoreMethods = ['format'];
	this._fixedSize = 'full';
	this._properties = ['select', 'selectPrefix', 'autoCleanup', 'publicID', 'folder', 'filenameAsPublicID'];

	multiimages.super_.call(this, list, path, options);

	// validate cloudinary config
	if (!keystone.get('cloudinary config')) {
		throw new Error('Invalid Configuration\n\n'
			+ 'MultiImages fields (' + list.key + '.' + this.path + ') require the "cloudinary config" option to be set.\n\n'
			+ 'See http://keystonejs.com/docs/configuration/#services-cloudinary for more information.\n');
	}
}
multiimages.properName = 'MultiImages';
util.inherits(multiimages, FieldType);

/**
 * Gets the folder for images in this field
 */
multiimages.prototype.getFolder = function () {
	var folder = null;
 ...
 */
multiimages.prototype.addToSchema = function (schema) {
 ...
 */
multiimages.prototype.format = function (item) {
 ...
 */
multiimages.prototype.getData = function (item) {
 ...
 * Deprecated
 */
multiimages.prototype.inputIsValid = function (data) { // eslint-disable-line no-unused-vars
	// TODO - how should image field input be validated?
	return true;
};


multiimages.prototype._originalGetOptions = multiimages.prototype.getOptions;

multiimages.prototype.getOptions = function () {
 ...
 */
multiimages.prototype.updateItem = function (item, data, files, callback) {
	...
	async.map(values, function (value, next) {
		if (typeof value === 'object' && 'public_id' in value) {
			...
			if(!value.path){
				...
				async.map(newvalues, function (newvalue, subnext) {				
					// File provided - upload it
					var uploadOptions = getUploadOptions();
					// NOTE: field.options.publicID has been deprecated (tbc)
					if (field.options.filenameAsPublicID && newvalue.originalname && typeof newvalue.originalname === 'string') {
						uploadOptions = assign({}, uploadOptions, {
							public_id: newvalue.originalname.substring(0, newvalue.originalname.lastIndexOf('.')),
						});
					}
					cloudinary.uploader.upload(newvalue.path, function (result) {
						if (result.error) {
							subnext(result.error);
						} else {
							subnext(null, result);
						}
					}, uploadOptions);
